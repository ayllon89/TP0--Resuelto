...
int main(void) {
	...
	int cliente_fd = esperar_cliente(server_fd);//el programa se queda trabado aca hasta que alguien se conecte,aca se hace un accept,
												//una vez que se hace accept no se puede conectar otro cliente hasta que termine de ejecutarse el while de abajo porque el servidor no esta en el estado accept
